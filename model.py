...
class HierarchialAttentionNetwork(nn.Module):
    ...
    def __init__(self, n_classes, vocab_size, emb_size, word_rnn_size, sentence_rnn_size, para_rnn_size, word_rnn_layers,
        ...
        self.para_attention = ParagraphAttention(vocab_size, emb_size, word_rnn_size, sentence_rnn_size, para_rnn_size,
                                                    ...
                                                    sentence_att_size, para_att_size, dropout)
        # Classifier
        if edl:
            self.fc = nn.Linear((2 * para_rnn_size), n_classes*2)
        else:
            self.fc = nn.Linear((2 * para_rnn_size), n_classes)
        self.relu = nn.ReLU()
    ...
    def forward(self, X):
        documents, para_per_document, sentences_per_para, words_per_sentence = X[0], X[1], X[2], X[3]
        """
        documents = documents[:,0,:,:] (10,4,20)
        sentences_per_para = sentences_per_para[:,0] (10)
        words_per_sentence = words_per_sentence[:,0,:] (10,4)
        Forward propagation.
...
class ParagraphAttention(nn.Module):
    ...
    def __init__(self, vocab_size, emb_size, word_rnn_size, sentence_rnn_size, para_rnn_size, word_rnn_layers, sentence_rnn_layers,
                 para_rnn_layers, word_att_size, sentence_att_size, para_att_size, dropout):
        """
        ...
        self.para_rnn = nn.GRU(2 * sentence_rnn_size, para_rnn_size, num_layers=para_rnn_layers,
    ...
    def forward(self, documents, para_per_document, sentences_per_para, words_per_sentence):
